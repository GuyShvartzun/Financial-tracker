...
export default function Header({ 
  ...
  return (
    <header className="bg-[#FFFFFF]/95 border-b border-[#E8E2D8] sticky top-0 z-40 backdrop-blur-md px-3 sm:px-4 py-2.5 sm:py-3 shadow-xs font-sans dir-rtl text-right" dir="rtl">
      <div className="max-w-7xl mx-auto space-y-2 sm:space-y-2.5">
        
        {/* Top Tier: Logo & Cloud Status on right, User Profile & Controls on left */}
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          
          {/* Logo & App Title */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="bg-[#E8F5E9] text-[#2E7D32] border border-[#C8E6C9] w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl shadow-xs font-black text-base sm:text-xl flex items-center justify-center shrink-0 select-none">
              ₪
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                {isCloudSynced && (
                  <span className="text-[9px] sm:text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 px-1.5 py-0.5 rounded-full font-bold whitespace-nowrap shrink-0 flex items-center gap-1">
                    <Cloud className="w-2.5 h-2.5" />
                    <span className="hidden xs:inline">מחובר</span>
                  </span>
                )}
              </div>
            </div>
          ...
          {/* User Profile, Privacy Mode Toggle & Logout */}
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              className={`p-1.5 sm:px-2.5 sm:py-1.5 rounded-xl text-xs font-bold transition border cursor-pointer flex items-center gap-1.5 ${
                isPrivacyMode
            ...
            >
              {isPrivacyMode ? (
                <>
                  ...
                  <span className="hidden md:inline text-[11px]">פרטיות פעילה</span>
                </>
              ) : (
                <>
                  <Eye className="w-3.5 h-3.5 text-stone-500" />
                  <span className="hidden md:inline text-[11px]">מצב פרטיות</span>
                </>
              )}
            </button>

            {/* Profile Bar */}
            <div className="flex items-center gap-1.5 sm:gap-2 bg-[#FAF7F2] p-1 sm:p-1.5 rounded-xl border border-[#E8E2D8]">
              ...
              <button 
                ...
                onClick={onLogout}
                title="התנתק מהחשבון"
                className="text-[10px] sm:text-xs bg-[#FFEBEE] hover:bg-[#FFCDD2] text-[#C62828] font-bold px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg border border-[#EF9A9A] transition shadow-2xs cursor-pointer whitespace-nowrap flex items-center gap-1"
              >
                <LogOut className="w-3 h-3" />
                <span className="hidden xs:inline">התנתק</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-1.5 pt-1 border-t border-[#E8E2D8]/60 text-xs">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <button
                ...
                className="text-[10px] sm:text-[11px] font-bold text-stone-600 hover:text-stone-900 bg-[#FAF7F2] hover:bg-[#F2ECE1] px-2 py-0.5 rounded-md border border-[#DDD6CA] transition cursor-pointer flex items-center gap-1.5"
